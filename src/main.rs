...
fn run(args: Arguments) {
    ...
    let current_number_sequence = Arc::new(Mutex::new(0));
    while *numbers_found.lock().unwrap() < args.f {
        ...
        pool.execute(move || {
            ...
            *current_number_sequence += 1;
            println!("{}", current_number_sequence);
            let hash = digest(current_number_sequence.to_string());
            let is_hash_valid = hash.chars().rev().take(args.n).all(|char| char == '0');
            if is_hash_valid == true && *numbers_found.lock().unwrap() < args.f {
                current_state.push(HashPair(*current_number_sequence, hash));
...
fn test_run(args: Arguments) {
    ...
    let mut current_number_sequence = 1;
    while numbers_found < args.f {
        let hash = digest(current_number_sequence.to_string());
        let is_hash_valid = hash.chars().rev().take(args.n).all(|char| char == '0');
        if is_hash_valid == true {
            println!("test {}: {}", current_number_sequence, hash);
            result.push(HashPair(current_number_sequence, hash));
            numbers_found += 1;
        }
        current_number_sequence += 1;
    }
